...
FootTrajectoryBezier::FootTrajectoryBezier()
    : Ax(Vector6::Zero()), Ay(Vector6::Zero()), Az(Vector7::Zero()),
      intersectionPoint_(Vector2::Zero()), t_swing_(0.), useBezier(true),
      elevation_(ElevationType(HORIZONTAL)), ineq_(Vector3::Zero()),
      ineq_vector_(0.), margin_max_up_(0.), margin_max_down_(0.),
      t_margin_up_(0.), t_margin_down_(0.), t_stop_up_(0.), t_stop_down_(0.),
      z_margin_up_(0.), z_margin_down_(0.) {
  pDef.degree = 7;
  pDef.flag = optimization::INIT_POS | optimization::END_POS |
  ...
  problem_data_t pbData =
      optimization::setup_control_points<pointX_t, double, safe>(pDef);
...
FootTrajectoryBezier::FootTrajectoryBezier(const FootTrajectoryBezier &other)
    : Ax(other.Ax), Ay(other.Ay), Az(other.Az),
      ...
      starting_position_(other.starting_position_), t_swing_(other.t_swing_),
      useBezier(other.useBezier), maxHeight_(other.maxHeight_), t0_(other.t0_),
      elevation_(other.elevation_), N_samples(other.N_samples),
      N_samples_ineq(other.N_samples_ineq), degree(other.degree),
      res_size(other.res_size), pDef(other.pDef), fitBezier(other.fitBezier),
      curve_(other.curve_), P_(other.P_), q_(other.q_), G_(other.G_),
      h_(other.h_), C_(other.C_), d_(other.d_), x(other.x), ineq_(other.ineq_),
      ineq_vector_(other.ineq_vector_), margin_max_up_(other.margin_max_up_),
      margin_max_down_(other.margin_max_down_),
      t_margin_up_(other.t_margin_up_), t_margin_down_(other.t_margin_down_),
      t_stop_up_(other.t_stop_up_), t_stop_down_(other.t_stop_down_),
      z_margin_up_(other.z_margin_up_), z_margin_down_(other.z_margin_down_),
      margin_adapted_(other.margin_adapted_), EPS_(other.EPS_),
      expected(other.expected), status(other.status), qp(other.qp) {}

void FootTrajectoryBezier::initialize(int const &N_samples_in,
                                      int const &N_samples_ineq_in,
                                      int const &degree_in,
                                      double const &t_swing,
...
void FootTrajectoryBezier::set_parameters_up(double const &margin_max_in,
...
void FootTrajectoryBezier::set_parameters_down(double const &margin_max_in,
                                               double const &t_margin_in,
                                               double const &z_margin_in) {
...
void FootTrajectoryBezier::create_simple_curve(Vector3 const &pos_init,
                                               Vector3 const &vel_init,
                                               Vector3 const &pos_end,
                                               double t0) {
  ...
  if (t0 < 10e-4) {
    ...
    updatePolyCoeff_XY(pos_init, Vector3::Zero(), Vector3::Zero(), pos_end, 0.,
                       t_swing_);
  ...
  for (int j = 0; j < N_samples; j++) {
    ...
    q_ += linear_var.B().transpose() *
          (linear_var.c() - evaluatePoly(0, t0 + (t_swing_ - t0) * t_b_));
  ...
  curve_ = bezier_t::bezier_curve_t(fitBezier.waypoints().begin(),
                                    fitBezier.waypoints().end(), t0, t_swing_);
}

void FootTrajectoryBezier::updateInequalityUp(Vector2 const &pos_init,
                                              Vector2 const &pos_end,
      ...
      if (abs(Q1[0] - Q2[0]) >= 0.001) {
        ...
        if (pos_end[1] - a * pos_end[0] - b >= 0) {
          ...
          if (ineq_.head(2).transpose() * pos_init >= b2) {
            ...
            ineq_vector_ = pos_init[1] - a * pos_init[0] + EPS_ -
                           margin_adapted_ * std::sqrt(1 + std::pow(a, 2));
          } else {
            ineq_vector_ = b2;
          }
          // ineq_vector_ = b2;
...
void FootTrajectoryBezier::updateInequalityDown(Vector2 const &pos_init,
                                                Vector2 const &pos_end,
                                                Surface const &surface) {
  int nb_vert = (int)surface.vertices_.rows();
  MatrixN vert = surface.vertices_;
      ...
      if (abs(Q1[0] - Q2[0]) >= 0.001) {
        ...
        if (pos_end[1] - a * pos_end[0] - b >= 0) {
          ...
          if (ineq_.head(2).transpose() * pos_end >= -b2) {
            ineq_vector_ = -(pos_end[1] - a * pos_end[0] - EPS_ +
                             margin_adapted_ * std::sqrt(1 + std::pow(a, 2)));
          } else {
            ineq_vector_ = -b2;
          }
...
void FootTrajectoryBezier::update(Vector3 const &pos_init,
                                  Vector3 const &vel_init,
  ...
  case UP: {
    ...
    if (((x_t[2] < pos_end(2)) && ineq_vector_ != 0.) ||
        (t0_ < t_stop_up_ + t_margin_up_)) {
      updateInequalityUp(pos_init.head(2), pos_end.head(2), surface_end);

      double z_margin = (pos_end(2) - starting_position_(2)) *
                        z_margin_up_; // 10% around the limit height
      double t_s;
      double zt;
  ...
  case DOWN: {
    ...
    double z_margin = (starting_position_(2) - pos_end(2)) *
                      z_margin_down_; // 10% around the limit height
    ...
    updateInequalityDown(starting_position_.head(2), pos_end.head(2),
                         surface_init);
    ...
    if ((x_t[2] >= starting_position_(2)) &&
        ((t_swing_ - t0_) >= t_stop_down_ - t_margin_down_)) {
...
Vector3 FootTrajectoryBezier::evaluateBezier(int const &indice,
                                             double const &t) {
  if (indice == 0) {
    return curve_(t);
  } else {
...
Vector3 FootTrajectoryBezier::evaluatePoly(int const &indice, double const &t) {
  ...
  if (indice == 0) {
    double x = Ax(0) + Ax(1) * t + Ax(2) * std::pow(t, 2) +
               Ax(3) * std::pow(t, 3) + Ax(4) * std::pow(t, 4) +
               Ax(5) * std::pow(t, 5);
    double y = Ay(0) + Ay(1) * t + Ay(2) * std::pow(t, 2) +
               Ay(3) * std::pow(t, 3) + Ay(4) * std::pow(t, 4) +
               Ay(5) * std::pow(t, 5);
    double z = Az(0) + Az(1) * t + Az(2) * std::pow(t, 2) +
               Az(3) * std::pow(t, 3) + Az(4) * std::pow(t, 4) +
...
void FootTrajectoryBezier::updatePolyCoeff_XY(
    Vector3 const &x_init, Vector3 const &v_init, Vector3 const &a_init,
  ...
  Ax(5) = (ddx0 * std::pow(t, 2) - 2 * ddx0 * t * d - 6 * dx0 * t +
  ...
  Ax(4) =
      ...
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ax(3) = (std::pow(t, 4) * ddx0 + 3 * std::pow(d, 4) * ddx0 -
           8 * std::pow(t, 3) * dx0 + 12 * std::pow(d, 3) * dx0 +
           20 * std::pow(t, 2) * x0 - 20 * std::pow(t, 2) * x1 +
           20 * std::pow(d, 2) * x0 - 20 * std::pow(d, 2) * x1 +
           80 * t * d * x0 - 80 * t * d * x1 + 4 * std::pow(t, 3) * d * ddx0 +
           28 * t * std::pow(d, 2) * dx0 - 32 * std::pow(t, 2) * d * dx0 -
           8 * std::pow(t, 2) * std::pow(d, 2) * ddx0) /
          (2 * std::pow((t - d), 2) *
           (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
            std::pow(d, 3)));
  Ax(2) = -(std::pow(d, 5) * ddx0 + 4 * t * std::pow(d, 4) * ddx0 +
  ...
  Ax(1) =
      ...
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ax(0) = (2 * x1 * std::pow(t, 5) - ddx0 * std::pow(t, 4) * std::pow(d, 3) -
           10 * x1 * std::pow(t, 4) * d +
  ...
  Ay(5) = (ddy0 * std::pow(t, 2) - 2 * ddy0 * t * d - 6 * dy0 * t +
  ...
  Ay(4) =
      ...
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ay(3) = (std::pow(t, 4) * ddy0 + 3 * std::pow(d, 4) * ddy0 -
           8 * std::pow(t, 3) * dy0 + 12 * std::pow(d, 3) * dy0 +
           20 * std::pow(t, 2) * y0 - 20 * std::pow(t, 2) * y1 +
           20 * std::pow(d, 2) * y0 - 20 * std::pow(d, 2) * y1 +
           80 * t * d * y0 - 80 * t * d * y1 + 4 * std::pow(t, 3) * d * ddy0 +
           28 * t * std::pow(d, 2) * dy0 - 32 * std::pow(t, 2) * d * dy0 -
           8 * std::pow(t, 2) * std::pow(d, 2) * ddy0) /
          (2 * std::pow((t - d), 2) *
           (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
            std::pow(d, 3)));
  Ay(2) = -(std::pow(d, 5) * ddy0 + 4 * t * std::pow(d, 4) * ddy0 +
  ...
  Ay(1) =
      ...
      (2 * std::pow((t - d), 2) *
       (std::pow(t, 3) - 3 * std::pow(t, 2) * d + 3 * t * std::pow(d, 2) -
        std::pow(d, 3)));
  Ay(0) = (2 * y1 * std::pow(t, 5) - ddy0 * std::pow(t, 4) * std::pow(d, 3) -
           10 * y1 * std::pow(t, 4) * d +
...
void FootTrajectoryBezier::updatePolyCoeff_Z(Vector3 const &x_init,
...
bool FootTrajectoryBezier::doIntersect_segment(Vector2 const &p1,
                                               Vector2 const &q1,
                                               Vector2 const &p2,
                                               Vector2 const &q2) {
...
bool FootTrajectoryBezier::onSegment(Vector2 const &p, Vector2 const &q,
                                     Vector2 const &r) {
...
int FootTrajectoryBezier::orientation(Vector2 const &p, Vector2 const &q,
                                      Vector2 const &r) {
  double val =
...
void FootTrajectoryBezier::get_intersect_segment(Vector2 a1, Vector2 a2,
