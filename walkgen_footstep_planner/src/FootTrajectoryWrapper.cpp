...
FootTrajectoryWrapper::FootTrajectoryWrapper(double dt, int N,
                                             double step_height,
                                             const pinocchio::SE3 &M_current,
                                             const pinocchio::SE3 &M_next)
    : dt_(dt), N_(N), step_height_(step_height), M_current_(M_current),
      M_next_(M_next), M_tmp_(pinocchio::SE3::Identity()),
      Vel_tmp_(pinocchio::Motion::Zero()) {
  double t_swing = dt_ * static_cast<double>(N_);
  // Initialise the Bezier curve
  curve_->initialize(params_.N_sample, params_.N_sample_ineq, params_.degree,
                     t_swing, step_height_);
  curve_->set_parameters_up(params_.margin_up, params_.t_margin_up,
                            params_.z_margin_up);
  curve_->set_parameters_down(params_.margin_down, params_.t_margin_down,
                              params_.z_margin_down);
  curve_->create_simple_curve(M_current.translation(), Vector3::Zero(),
                              M_next.translation(), 0.);
}
...
FootTrajectoryWrapper::FootTrajectoryWrapper(double dt, int N,
    ...
    : dt_(dt), N_(N), step_height_(step_height), M_current_(M_current),
      M_next_(M_next), M_tmp_(pinocchio::SE3::Identity()),
      Vel_tmp_(pinocchio::Motion::Zero()), params_(params) {
...
pinocchio::SE3 FootTrajectoryWrapper::position(int k) {
  if (k >= N_) {
    throw std::invalid_argument(
        "Invalid argument: k is bigger than the allocated number of nodes.");
  }
...
void FootTrajectoryWrapper::update(const Vector3 &x0, const Vector3 &v0,
                                   const Vector3 &xf, double t0,
                                   const Surface &init_surface,
...
FootTrajectoryWrapper::FootTrajectoryWrapper(
    const FootTrajectoryWrapper &other) {
...
double FootTrajectoryWrapper::getT0() { return curve_->getT0(); }