...
ContactPhase::ContactPhase(const ContactPhase &other)
    : T_(other.T_), contactType_(other.contactType_) {
...
void ContactPhase::setTrajectory(
    std::shared_ptr<FootTrajectoryWrapper> trajectory) {
  this->trajectory_ = std::move(trajectory);
}

// Operator overload to add two contact phases together
std::shared_ptr<ContactPhase> ContactPhase::
operator+(const ContactPhase &phase) const {
  if (contactType_ != phase.contactType_) {
    throw std::invalid_argument("Couldn't append the contact phase since it "
                                "doesn't belong to the same type");
  }
  std::shared_ptr<FootTrajectoryWrapper> wrapper_cp =
      std::make_shared<FootTrajectoryWrapper>(*phase.trajectory_);
  return std::make_shared<ContactPhase>(T_ + phase.T_, contactType_,
                                        std::move(wrapper_cp));
}
