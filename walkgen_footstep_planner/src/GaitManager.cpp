...
void GaitManager::initialize(const pinocchio::Model& model,const VectorN& q){
    ...
    if (type_ == "trot")
    {
        initial_schedule_ = gait_generator_.trot({cs0, cs1},
                                          50,
                                          N_ss_,
                                          N_uss_,
                                          N_uds_,
                                          ...
                                          true,
                                          false);
        default_schedule_ = gait_generator_.trot({cs0, cs1},
                                          N_ds_,
                                          N_ss_,
                                          N_uss_,
                                          N_uds_,
    ...
    {
        initial_schedule_ = gait_generator_.walk({cs0, cs1},
                                          100,
                                          N_ss_,
                                          N_uss_,
                                          N_uds_,
                                          ...
                                          true,
                                          false);
        default_schedule_ = gait_generator_.walk({cs0, cs1},
                                          N_ds_,
                                          N_ss_,
                                          N_uss_,
                                          N_uds_,
...
std::vector<int> GaitManager::evaluate_config(const ContactSchedule& schedule, int timeline) {
    ...
    for (size_t c = 0; c < schedule.contactNames_.size() ; ++c) {
        ...
        size_t j = static_cast<size_t>(std::find(contactNames_sl1m_.begin(), contactNames_sl1m_.end(), name) - contactNames_sl1m_.begin());
...
void GaitManager::update_switches(std::map<int, std::vector<int>>& switches, const int timeline_in) {
    // int index_cs = 0;
    int T_total = 0;
                ...
                if (p + 1 < phases.size()) {
                    ...
                    if (switches.find(switch_active) == switches.end()) {
                        if ((T_active + T_inactive - 1) == cs.T_ - 1) {
                            ...
                            // TODO : A next_schedule mechanism with a wainting list populated by default if no CS.
                    ...
                    if (switch_inactive != -1 && switches.find(switch_inactive) == switches.end()) {
                        // switch_inactive = -1 --> Taken into account in the previous schedule.
...
MatrixN_int GaitManager::compute_gait(int timeline)
{
    ...
    // This function is called only when a new step is created. timeline == one of the switche key.
...
void GaitManager::print_queue(){
    ...
    ////////////////
    // Print header
    // const char* columns_str = std::getenv("COLUMNS"); // if COLUMNS is in the ENV variables.
    // int terminal_width = columns_str ? std::atoi(columns_str) : 100;
...
std::vector<std::vector<std::vector<CoeffBezier>>> GaitManager::get_coefficients() {
    ...
    for (auto itr = queue_cs_.rbegin(); itr != queue_cs_.rend(); ++itr) {
        ...
        for (size_t c = 0; c < cs.contactNames_.size(); ++c) {
            ...
            for (size_t p = 0; p < phases.size(); p += 2) {
                if (p + 1 < phases.size()) {
                    foot_list.push_back(CoeffBezier(phases[p + 1]->trajectory_->getT0(), phases[p + 1]->trajectory_->get_coefficients()));
                }
            }
...
//         // For each line in total line per schedule, check what should be printed
//         for (int iline = 0; iline < totalLines; iline++)
//         {
//             for (const auto &foot_name : contactNames_sl1m_)
//             {
//                 auto it = std::find(cs.contactNames_.begin(), cs.contactNames_.end(), foot_name);
...
//                             std::string text = "Active : " + std::to_string(phases[p]->T_);
//                             size_t padding = static_cast<size_t>(colWidth) - text.length();
//                             size_t leftPadding = padding / 2;
//                             size_t rightPadding = padding - leftPadding;
//                             std::cout << std::string(leftPadding, ' ') << text << std::string(rightPadding, ' ');
...
//                             std::string text = "Inactive : " + std::to_string(phases[p+1]->T_);
//                             size_t padding = static_cast<size_t>(colWidth) - text.length();
//                             size_t leftPadding = padding / 2;
//                             size_t rightPadding = padding - leftPadding;
//                             std::cout << std::string(leftPadding, ' ') << text << std::string(rightPadding, ' ');
...
// Eigen::Matrix<double, 2, 4> GaitManager::swicthes_to_gait(const std::map<int, std::vector<int>> &switches)
// {
    // Current gait :
    // std::vector<std::vector<int>> gait;
    // int timeline = 0;
