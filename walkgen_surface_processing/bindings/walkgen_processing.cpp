...
struct AffordanceLoaderPythonVisitor : public bp::def_visitor<AffordanceLoaderPythonVisitor<AffordanceLoader>> {
  ...
  void visit(PyClassAffordanceLoader &cl) const {
    cl.def(bp::init<>(bp::arg(""), "Default constructor."))
        .def(bp::init<double, double, double, char *>(bp::args("margin", "nbTriMargin", "minArea", "affordanceName"),
...
void exposePoint() {
  bp::class_<Point, boost::noncopyable>("Point",
                                        bp::init<double, double>(bp::args("x", "y"), "Constructor for point class."))
...
void exposeBayazit() {
  ...
  walkgen::python::StdVectorPythonVisitor<PointPtr, std::allocator<PointPtr>>::expose("StdVec_PointPtr");
  walkgen::python::StdVectorPythonVisitor<PointVecPtr, std::allocator<PointVecPtr>>::expose("StdVecVec_PointPtr");

  walkgen::python::StdVectorPythonVisitor<Point, std::allocator<Point>>::expose("StdVec_Point");
  walkgen::python::StdVectorPythonVisitor<std::vector<Point>, std::allocator<std::vector<Point>>>::expose(
      "StdVecVec_Point");

  bp::class_<Bayazit, boost::noncopyable>("Bayazit", bp::init<>(bp::args(""), "Constructor for Bayazit algorithm."))
...
BOOST_PYTHON_MODULE(libwalkgen_surface_processing_pywrap) {
  ...
  walkgen::python::StdVectorPythonVisitor<Vector3, std::allocator<Vector3>, true>::expose("StdVec_Vector3d");
